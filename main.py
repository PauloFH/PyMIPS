...
for line in asm: #simulando a atividade do pc
    if line.find(':') > 0: #quando encontrar um label na linha, a parte sem a label será escrita na string de funções 
        labelchar = False
        label = ""
        strf = ""
        for x in line.strip():
            if not labelchar:
                if x == ':':
                    labels.append(label) #acrescentando a label a lista de labels
                    inL.append(address) #acrescentando o endereço correspondente ao label na lista
                    labelchar = True
                else:
                    label += x
            else:
                strf = line.strip(label)
                label += ':' #acrescentando os ":" ao final do label
